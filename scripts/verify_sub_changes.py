...
with open("scratch/verify_sub_changes.php", "w") as f:
    f.write(php_script)

subprocess.run(["scp", "-i", SSH_KEY, "scratch/verify_sub_changes.php", f"{HOST}:/tmp/verify_sub_changes.php"], check=True)
res = subprocess.run(["ssh", "-i", SSH_KEY, HOST, "sudo docker cp /tmp/verify_sub_changes.php coolify:/var/www/html/verify_sub_changes.php && sudo docker exec coolify php /var/www/html/verify_sub_changes.php && sudo docker exec coolify rm -f /var/www/html/verify_sub_changes.php && rm -f /tmp/verify_sub_changes.php"], capture_output=True, text=True)
print(res.stdout)
if res.stderr:
    print("STDERR:")
    print(res.stderr)
