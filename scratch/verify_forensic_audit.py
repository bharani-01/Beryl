...
script_remote = "/tmp/verify_audit_remote.php"
with open("scratch/verify_audit_remote.php", "w", encoding="utf-8") as f:
    f.write(php_script)

subprocess.run(["scp", "-o", "StrictHostKeyChecking=no", "-i", SSH_KEY, "scratch/verify_audit_remote.php", f"{HOST}:{script_remote}"], check=True)
run_ssh(f"sudo docker cp {script_remote} coolify:/var/www/html/verify_audit_remote.php")
res = run_ssh("sudo docker exec coolify php /var/www/html/verify_audit_remote.php")
print(res.stdout)
if res.stderr:
    print("STDERR:", res.stderr)
run_ssh("sudo docker exec coolify rm -f /var/www/html/verify_audit_remote.php")
