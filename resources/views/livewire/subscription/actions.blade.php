<div wire:init="loadRefundEligibility" class="application-settings-workspace flex flex-col gap-6" x-data="{
    ...
    {{-- Plan Overview & Resource Limits Dashboard --}}
    <x-application.settings-section title="Plan overview & resource limits"
        description="Review your team's active subscription, container resource allocations, and volume quotas.">
        <x-slot:actions>
            <div class="flex items-center gap-2">
                @if ($isPaid)
                    <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-300">
                        <span class="size-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                        Active &amp; Paid ({{ $planName }})
                    </span>
                @elseif ($isOnTrial)
                    <span class="inline-flex items-center gap-1.5 rounded-full bg-sky-500/10 px-2.5 py-0.5 text-[11px] font-semibold text-sky-600 dark:bg-sky-500/20 dark:text-sky-300">
                        <span class="size-1.5 rounded-full bg-sky-500"></span>
                        14-day trial ({{ $trialDaysLeft }} {{ Str::plural('day', $trialDaysLeft) }} left)
                    </span>
                @else
                    <span class="inline-flex items-center gap-1.5 rounded-full bg-rose-500/10 px-2.5 py-0.5 text-[11px] font-semibold text-rose-600 dark:bg-rose-500/20 dark:text-rose-300">
        ...
        {{-- Limit Reached Alert Callout --}}
        @if ($isResourceLimitReached)
            <x-callout type="danger" title="Active resource limit reached" class="mb-4">
                Your team has reached the maximum allowed running resources for the <strong>{{ $planName }}</strong> plan ({{ $runningResources }} of {{ $maxApps }} running). Any further deploy or start attempts will be blocked until you upgrade your plan or stop existing resources.
            </x-callout>
        @elseif ($isVolumeLimitReached)
            <x-callout type="warning" title="Storage volume limit reached" class="mb-4">
                Your team has used all {{ $maxVolumes }} persistent storage volumes allowed on the {{ $planName }} plan. Upgrade your plan to attach additional persistent volumes.
            </x-callout>
        @endif

        {{-- Quota Metrics Cards Grid --}}
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {{-- Active Resources --}}
            <div class="rounded-xl border border-neutral-200/80 bg-white/50 p-3.5 shadow-2xs dark:border-white/[0.08] dark:bg-white/[0.035]">
                <div class="flex items-center justify-between text-[11px] font-medium text-neutral-500 dark:text-fg-faint uppercase tracking-wider">
                    <span>Active Resources</span>
                    <x-reicon name="servers" class="size-4 text-neutral-400 dark:text-fg-muted" />
                </div>
                <div class="mt-2 flex items-baseline gap-1.5">
                    <span class="text-2xl font-bold tracking-tight {{ $isResourceLimitReached ? 'text-rose-600 dark:text-rose-400' : 'text-neutral-900 dark:text-fg' }}">
                        {{ $runningResources }}
                    </span>
                    <span class="text-sm font-medium text-neutral-500 dark:text-fg-dim">
                        / {{ $maxApps > 0 ? $maxApps : 'Unlimited' }}
                    </span>
                </div>
                <div class="mt-2">
                    @if ($maxApps > 0)
                        @php
                            $pct = min(100, round(($runningResources / max(1, $maxApps)) * 100));
                        @endphp
                        <div class="h-1.5 w-full rounded-full bg-neutral-200/70 dark:bg-white/[0.08] overflow-hidden">
                            <div class="h-full rounded-full transition-all duration-300 {{ $isResourceLimitReached ? 'bg-rose-500' : ($pct > 75 ? 'bg-amber-500' : 'bg-emerald-500') }}"
                                style="width: {{ $pct }}%"></div>
                        </div>
                        <p class="mt-1.5 text-[11px] text-neutral-500 dark:text-fg-dim">
                            @if ($isResourceLimitReached)
                                <span class="font-semibold text-rose-600 dark:text-rose-400">Limit reached (0 slots free)</span>
                            @else
                                {{ max(0, $maxApps - $runningResources) }} free {{ Str::plural('slot', max(0, $maxApps - $runningResources)) }} available
                            @endif
                        </p>
                    @else
                        <p class="mt-1.5 text-[11px] text-emerald-600 dark:text-emerald-400 font-medium">
                            Unlimited deployment capacity
                        </p>
                    @endif
                </div>
            </div>
            ...
            <div class="rounded-xl border border-neutral-200/80 bg-white/50 p-3.5 shadow-2xs dark:border-white/[0.08] dark:bg-white/[0.035]">
                <div class="flex items-center justify-between text-[11px] font-medium text-neutral-500 dark:text-fg-faint uppercase tracking-wider">
                    <span>CPU Limit</span>
                    <svg class="size-4 text-neutral-400 dark:text-fg-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        <rect x="4" y="4" width="16" height="16" rx="2"/><rect x="9" y="9" width="6" height="6"/>
                        <path d="M9 1v3M15 1v3M9 20v3M15 20v3M20 9h3M20 14h3M1 9h3M1 14h3"/>
                    </svg>
                </div>
                <div class="mt-2 flex items-baseline gap-1.5">
                    <span class="text-2xl font-bold tracking-tight text-neutral-900 dark:text-fg">
                        {{ $cpuLimit }}
                    </span>
                    <span class="text-sm font-medium text-neutral-500 dark:text-fg-dim">
                        vCPU
                ...
                <p class="mt-3 text-[11px] text-neutral-500 dark:text-fg-dim">
                    Per-container compute allocation
                </p>
            </div>
            ...
            <div class="rounded-xl border border-neutral-200/80 bg-white/50 p-3.5 shadow-2xs dark:border-white/[0.08] dark:bg-white/[0.035]">
                <div class="flex items-center justify-between text-[11px] font-medium text-neutral-500 dark:text-fg-faint uppercase tracking-wider">
                    ...
                    <svg class="size-4 text-neutral-400 dark:text-fg-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 10h18M3 14h18M5 6v12M19 6v12M9 10v4M12 10v4M15 10v4" />
                    </svg>
                </div>
                <div class="mt-2 flex items-baseline gap-1.5">
                    <span class="text-2xl font-bold tracking-tight text-neutral-900 dark:text-fg">
            ...
            <div class="rounded-xl border border-neutral-200/80 bg-white/50 p-3.5 shadow-2xs dark:border-white/[0.08] dark:bg-white/[0.035]">
                <div class="flex items-center justify-between text-[11px] font-medium text-neutral-500 dark:text-fg-faint uppercase tracking-wider">
                    ...
                    <x-reicon name="storages" class="size-4 text-neutral-400 dark:text-fg-muted" />
                </div>
                <div class="mt-2 flex items-baseline gap-1.5">
                    <span class="text-2xl font-bold tracking-tight {{ $isVolumeLimitReached ? 'text-rose-600 dark:text-rose-400' : 'text-neutral-900 dark:text-fg' }}">
                        {{ $volumesCount }}
                    </span>
                    <span class="text-sm font-medium text-neutral-500 dark:text-fg-dim">
                        / {{ $maxVolumes > 0 ? $maxVolumes : 'Unlimited' }} vols
                    </span>
                </div>
                <p class="mt-3 text-[11px] text-neutral-500 dark:text-fg-dim truncate" title="{{ $storageLimit }} disk quota">
                    {{ $storageLimit }} disk quota
                </p>
            </div>
        </div>
